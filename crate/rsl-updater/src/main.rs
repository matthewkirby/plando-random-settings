...
async fn main(args: Args) -> Result<(), Error> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(concat!("rsl-updater/", env!("CARGO_PKG_VERSION")))
        .build()?;
    let release = Repo::new("matthewkirby", "plando-random-settings").latest_release(&client).await?.ok_or(Error::NoReleases)?;
